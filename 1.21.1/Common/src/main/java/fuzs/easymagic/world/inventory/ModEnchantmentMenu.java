...
public class ModEnchantmentMenu extends EnchantmentMenu implements ContainerListener {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        return QuickMoveRuleSet.of(this, this::moveItemStackTo)
                ...
                .addContainerSlotRule(0, (Slot slot) -> {
                    return this.isEnchantable(slot.getItem());
                })
                .addInventoryRules()
    ...
    /**
     * Alternative to {@link ItemStack#isEnchantable()}, and takes {@link Item#getEnchantmentValue()} into account.
     */
    private boolean isEnchantable(ItemStack itemStack) {
        if (!itemStack.getOrDefault(DataComponents.ENCHANTMENTS, ItemEnchantments.EMPTY).isEmpty()) {
            return false;
        } else if (!itemStack.getItem().isEnchantable(itemStack) && itemStack.getItem().getEnchantmentValue() <= 0) {
            return false;
        } else {
            return true;
        }
