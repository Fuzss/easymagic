...
public class AnvilTweaksElement extends AbstractElement implements ICommonElement {
    ...
    private void onRightClickBlock(final PlayerInteractEvent.RightClickBlock evt) {
        ...
        if (evt.getSide().isServer() && (this.repairWithBlock && isRepairMaterial || this.repairWithIngot && isRepairChanceMaterial)) {
            ...
            if (blockState != null) {
                ...
                if (isRepairMaterial && blockState.getBlock() == Blocks.CHIPPED_ANVIL) {
    ...
    private static BlockState repair(BlockState state) {
        ...
        if (state.getBlock() == Blocks.DAMAGED_ANVIL) {
            ...
            return state.getBlock() == Blocks.CHIPPED_ANVIL ? Blocks.ANVIL.getDefaultState().with(AnvilBlock.FACING, state.get(AnvilBlock.FACING)) : null;
        }
    }
